...
ui <- navbarPage(
        ...
        tabsetPanel(
          tabPanel("Summary",
            ...
            fluidRow(
              ...
              column(3, uiOutput("p_card_strike")),
              column(3, uiOutput("p_card_zone")),
              column(3, uiOutput("p_card_velo"))
            ),
            br(),
            tags$div(class="section-header","Pitch Arsenal"),
            tags$div(class="section-sub","IVB/HB from trajectory. HAA/VAA = approach angle at the plate; Rel Angle = vertical release angle."),
  ...
  tabPanel("Staff",
    ...
    tags$div(class="section-sub","Every pitcher, all fall sessions. Click a column header to sort."),
    tags$div(class="filter-bar",
...
server <- function(input, output, session) {
  ...
  output$p_card_zone <- renderUI({ d <- p_filt(); req(d); stat_card(pct(mean(d$ZoneCheck,na.rm=TRUE)),"Zone%") })
  ...
  p_arsenal_df <- reactive({
    ...
    d %>% group_by(Pitch=PitchType) %>%
      summarise(
        ...
        `Zone%`=pct(mean(ZoneCheck,na.rm=TRUE)),
        .groups="drop"
      ) %>%
      ...
      dplyr::select(Pitch,Pitches,Usage,`Avg Velo`,`Max Velo`,Spin,`Spin Eff`,IVB,HB,
                    RelZ,RelX,`Rel Angle`,HAA,VAA,`Strike%`,`Zone%`) %>%
  ...
  p_seqMatrix_plot <- reactive({
    ...
    lbl <- c(usage="Usage%", avg_strike="Avg Strike%", second_strike="2nd Strike%",
             both_strike="Both Strikes%", zone_rate="2nd Zone%")[metric]
